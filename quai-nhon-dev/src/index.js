...
(function mountOperatorConsole(){
  ...
  function renderPipelineState(job){
    ...
    const completed=manifests.map(manifest=>!!manifest&&!failed(manifest));
    const failedSteps=manifests.map(manifest=>!!manifest&&failed(manifest));
    const currentIndex=completed.findIndex((done,index)=>!done&&!failedSteps[index]);
    if(["failed","error"].includes(String(job?.status||"").toLowerCase())&&currentIndex>=0)failedSteps[currentIndex]=true;
    const inProgressIndex=currentIndex<0||failedSteps[currentIndex]?null:currentIndex;
    const completedCount=completed.filter(Boolean).length;
    const failedCount=failedSteps.filter(Boolean).length;
    const inProgressCount=inProgressIndex===null?0:1;
    const pendingCount=pipelineSteps.length-completedCount-failedCount-inProgressCount;
    const progress=Math.round(completedCount/pipelineSteps.length*100);
    const steps=document.querySelectorAll("#steps .step");
    steps.forEach((element,index)=>{element.className="step "+(completed[index]?"completed":failedSteps[index]?"failed":index===inProgressIndex?"active":"pending")});
    const metrics=document.querySelectorAll(".metric strong");
